...
-opaque queue(DTB) :: {queue, list(DTB), list(DTB)}.

-spec new() -> queue(any()).
new() ->
    {queue, [], []}.

-spec from_list(list(DTI)) -> queue(DTI).
from_list(List) ->
    {queue, [], List}.

-spec to_list(queue(DTL)) -> list(DTL).
to_list(Queue) ->
...
-spec pop_front(queue(DUD)) -> {ok, {DUD, queue(DUD)}} | {error, nil}.
pop_front(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec reverse(queue(DUI)) -> queue(DUI).
reverse(Queue) ->
...
-spec is_logically_equal(queue(DUQ), queue(DUQ), fun((DUQ, DUQ) -> boolean())) -> boolean().
...
-spec is_equal(queue(DUT), queue(DUT)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
